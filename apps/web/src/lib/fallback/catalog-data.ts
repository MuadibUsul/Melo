...
function materializePlaylist(id: string) {
  const seed = playlistSeeds.find((item) => item.id === id);
  if (!seed) return null;
  return {
    id: seed.id,
    title: seed.title,
...
export function getSeedTracks() {
  return tracks.slice();
}

export function getSeedTrack(id: string) {
  return tracks.find((track) => track.id === id) ?? null;
}

export function getSeedTracksByCreator(creatorId: string) {
  const items = tracks.filter((track) => track.creator.id === creatorId);
  return { items, total: items.length };
}

export function getSeedPlaylist(id: string): SeedPlaylist | null {
  return materializePlaylist(id);
}

export function getSeedPublicPlaylists(): SeedPlaylist[] {
  return playlistSeeds
    .map((item) => materializePlaylist(item.id))
    .filter((item): item is SeedPlaylist => Boolean(item));
}

export function getSeedHotChart(limit = 50) {
  return tracks
    .slice()
    .sort((a, b) => b.playCount + b.likeCount * 2 - (a.playCount + a.likeCount * 2))
    .slice(0, limit)
    .map((track, index) => ({ ...track, rank: index + 1, score: track.playCount + track.likeCount * 2 }));
}

export function getSeedNewChart(limit = 50) {
  return tracks
    .slice()
    .sort((a, b) => +new Date(b.publishedAt) - +new Date(a.publishedAt))
    .slice(0, limit)
    .map((track, index) => ({ ...track, rank: index + 1, score: limit - index }));
}

export function getSeedGenreChart(genre: string, limit = 30) {
  return tracks
    .filter((track) => track.genre === genre)
    .sort((a, b) => b.playCount - a.playCount)
    .slice(0, limit)
    .map((track, index) => ({ ...track, rank: index + 1, score: track.playCount }));
}

export function getSeedCreatorChart(limit = 20) {
  return creators
    .map((creator) => {
      const items = tracks.filter((track) => track.creator.id === creator.id);
...
export function searchSeedTracks(query: string) {
  const keyword = query.trim().toLowerCase();
  if (!keyword) return tracks.slice();
